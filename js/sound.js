...
const sfx={
  ...
  ensureCtx(){if(!this.ready||!this.ctx)return false;if(this.ctx.state==='suspended')this.ctx.resume();return true},
  ...
  tone(freq,dur,type,vol){
    if(!this.ensureCtx())return;const t=this.ctx.currentTime;
  ...
  spell(){if(!this.ensureCtx())return;const t=this.ctx.currentTime;const o=this.ctx.createOscillator(),g=this.ctx.createGain();o.type='sine';o.frequency.setValueAtTime(300,t);o.frequency.exponentialRampToValueAtTime(1200,t+0.2);g.gain.value=0.2;g.gain.linearRampToValueAtTime(0,t+0.25);o.connect(g);g.connect(this.masterGain);o.start(t);o.stop(t+0.25)},
  arrow(){this.tone(1200,0.05,'triangle',0.4);this.tone(800,0.03,'triangle',0.3)},
  monDeath(){if(!this.ensureCtx())return;const t=this.ctx.currentTime;const o=this.ctx.createOscillator(),g=this.ctx.createGain();o.type='sawtooth';o.frequency.setValueAtTime(300,t);o.frequency.exponentialRampToValueAtTime(60,t+0.4);g.gain.value=0.15;g.gain.linearRampToValueAtTime(0,t+0.4);o.connect(g);g.connect(this.masterGain);o.start(t);o.stop(t+0.4)},
  levelUp(){
    if(!this.ensureCtx())return;const t=this.ctx.currentTime;
